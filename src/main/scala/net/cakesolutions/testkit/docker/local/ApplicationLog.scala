...
package net.cakesolutions.testkit.docker.local
...
object ApplicationLog extends LoggingSource[Json] {
  private val cmd = Seq("docker", "logs", "-f", "-t")
      ...
      try {
        scheduler.execute(new Runnable {
          def run(): Unit = {
            ...
            blocking {
              val process = Process(cmd :+ id).run(ProcessLogger(handleLogEvent, handleLogEvent))
